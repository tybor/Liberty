   ...
   wrap_on (a_stream: OUTPUT_STREAM)
      do
         if not has_wrapper then
            log("Variable `@(1)' does not have a wrapper type (address of query emitted anyway)%N",
            <<c_string_name>>)
            buffer.reset
            ...
            buffer.put_message(once "       address_of_@(1): POINTER%N%
                                %               -- Address of @(1)%N%
                                %               external %"plug_in%"%N%
                                %               alias %"{%N%
                                %                       location: %".%"%N%
         ...
         elseif not is_public then
            log(once "Skipping 'hidden' variable `@(1)'%N",
            <<c_string_name>>)
            buffer.put_message(once "%T-- `hidden' variable @(1) skipped.%N",
            <<c_string_name>>)
         elseif not namespace.is_main then
            ...
            log(once "Variable @(1)%N",
            <<c_string_name>>)
            buffer.put_message(once "%T@(1): @(2)%N%
                                %               -- @(1)%N%
                                %               external %"plug_in%"%N%
                                %               alias %"{%N%
                                %                       location: %".%"%N%
