...
feature {ANY} -- Setters
	...
	set_border is
		do
			value := value.bit_or(border_low_level)
		end

	unset_border is
		do
			value := value.bit_xor(border_low_level)
		end
...
feature {ANY} -- Queries
	...
	is_border: BOOLEAN is
		do
			Result := (value=border_low_level)
		end
...
feature {WRAPPER, WRAPPER_HANDLER} -- Low level values
	...
	border_low_level: INTEGER is
		external "plug_in"
 		alias "{
 			location: "."
 			module: "plugin"
 			feature_name: "GDK_DECOR_BORDER"
 			}"
 		end
